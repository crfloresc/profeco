 ...
 * - Use inconsistency route API
 * - Use to catch res
 */
app.get('/favicon.ico', (req, res) => res.status(204));
app.use('/api/v1', require('./routes/inconsistency.routes'));
...
app.listen(app.get('port'), () => {
  info('[server] app service inconsistency listening on '+ app.get('port'));
}).on('close', () => {
  info('[server] app service inconsistency stopped');
}).on('error', (err) => {
  error('[server] app service inconsistency error ' + err.message)
});
